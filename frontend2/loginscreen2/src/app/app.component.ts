  ...
  onReadall(){
   ...
   public  onUpdate(){
    ...
    deletedata(i){
      ...
      this.service.onDelete(this.list[i].name).subscribe((response:any)=>{
        if(response.success){
          alert(this.list[i].name+" " +"is deleted");
            this.wipeout=false;
          //this.name=null;
        }
      })
    ...
    editdata(i){
      ...
      this.gender=this.list[i].gender;
      this.adress=this.list[i].adress;
    }
